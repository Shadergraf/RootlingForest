...
float3 TransformPositionWSToOutlinePositionWS(float3 positionWS, float positionVS_Z, float3 normalWS)
{
    ...
    float outlineExpandAmount = _OutlineWidth * GetOutlineCameraFovAndDistanceFixMultiplier(positionVS_Z, _OutlineDistanceScale);
...
void ApplyOutlineToSurfaceData(inout ToonSurfaceData surface)
{
    surface.albedo *= _OutlineColor;
    surface.emission *= _OutlineEmissionColor;
}
...
#ifdef ToonShaderIsOutline
    ApplyOutlineToSurfaceData(surfaceData);
#endif
